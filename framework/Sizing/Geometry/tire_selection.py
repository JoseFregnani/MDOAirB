...
def tire_selection(load, V_qualified, pneu_pressure, selection_criteria):
    ...
    df_tires_meet_condition = df_tires[(df_tires.load >= load*1.1) & (df_tires.velocity >= speed) & (df_tires.pressure <= pneu_pressure)]
    
    if selection_criteria == 'weight':
        tire_selected = df_tires_meet_condition.loc[df_tires_meet_condition['weight']==df_tires_meet_condition['weight'].min(),:]
    elif selection_criteria == 'size':
        df_tire_min_size = df_tires_meet_condition.loc[df_tires_meet_condition['diameter']==df_tires_meet_condition['diameter'].min(),:]
        tire_selected = df_tires_meet_condition.loc[df_tires_meet_condition['weight']==df_tires_meet_condition['weight'].min(),:]
